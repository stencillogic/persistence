...
{
    return sizeof(pproto_state);
}

handle pproto_create(void *ss_buf, int sock)
{
    pproto_state *state = (pproto_state *)ss_buf;
...
{
    pproto_state *state = (pproto_state *)ss;
    state->sock = sock;
}

sint8 pproto_send_fully(handle ss, const void *data, uint64 sz)
{
    pproto_state *state = (pproto_state *)ss;
    ssize_t written;
...
sint8 pproto_read_portion(handle ss)
{
    pproto_state *state = (pproto_state *)ss;
    ssize_t readsz;
...
sint8 pproto_get(handle ss, uint8 *buf, uint32 sz)
{
    pproto_state *state = (pproto_state *)ss;
    uint32 cpsz, diff;
    ...
    while(sz > 0u)
    {
        ...
        {
            if(pproto_read_portion(ss) != 0) return 1;
        }
    }
    return 0;
}

sint8 pproto_flush_send(handle ss)
{
    pproto_state *state = (pproto_state *)ss;
    sint8 res = pproto_send_fully(ss, state->send_buf, state->send_buf_ptr);
    if(0 == res)
    {
...
sint8 pproto_send(handle ss, const uint8 *buf, uint32 sz)
{
    pproto_state *state = (pproto_state *)ss;
    ...
    while(sz > wr)
    {
        ...
        {
            if(pproto_send_fully(ss, state->send_buf, state->send_buf_ptr) != 0)
            {
                return 1;
            }
            state->send_buf_ptr = 0;
...
sint8 pproto_send_string(handle ss, const char* str)
{
    uint8 len;
    uint64 total = strlen(str), totaln;
    uint8 magic = PPROTO_LTEXT_STRING_MAGIC;

    if(0 != pproto_send(ss, (const uint8 *)&magic, sizeof(magic))) return 1;

    totaln = htobe64(total);
    if(0 != pproto_send(ss, (const uint8 *)&totaln, sizeof(totaln))) return 1;

    while(total > 0)
    {
        len = (total > 256) ? 256 : (uint8)total;
        if(pproto_send(ss, &len, sizeof(len)) != 0) return 1;
        if(pproto_send(ss, (const uint8 *)str, len*sizeof(uint8)) != 0) return 1;
        str += len;
        total -= len;
    }
...
sint8 pproto_read_recordset_col_desc(handle ss, pproto_col_desc *col_desc)
{
    ...
    uint64      sz=(MAX_TABLE_COL_NAME_LEN/256 + 1)*256;
    uint8       alias_buf[sz];

    if(0 != pproto_get(ss, &dt_magic, sizeof(dt_magic))) return 1;
    ...
    switch(dt_magic)
    {
        ...
        case PPROTO_LTEXT_STRING_MAGIC:
            ...
            if(0 != pproto_get(ss, (uint8 *)&dt_lenn, sizeof(dt_len))) return 1;
        ...
        case PPROTO_NUMERIC_VALUE_MAGIC:
            ...
            if(0 != pproto_get(ss, &dt_precision, sizeof(dt_precision))) return 1;
            if(0 != pproto_get(ss, &dt_scale, sizeof(dt_scale))) return 1;
    ...
    if(0 != pproto_read_str_begin(ss, &len)) return 1;
    if(0 != pproto_read_str(ss, alias_buf, &sz)) return 1;
    if(0 != pproto_read_str_end(ss)) return 1;
...
sint8 pproto_read_recordset_col_num(handle ss, uint16 *n)
{
    ...
    if(0 != pproto_get(ss, (uint8 *)&col_num, sizeof(col_num))) return 1;
...
sint8 pproto_recordset_start_row(handle ss, uint8 *nulls, uint32 nulls_sz)
{
    ...
    if(0 != pproto_get(ss, &magic, sizeof(magic))) return -1;
    ...
    {
        if(0 != pproto_get(ss, nulls, nulls_sz*sizeof(uint8))) return -1;
    }
    else
    {
        errno = EPROTO;
...
sint8 pproto_read_str_begin(handle ss, uint64 *len)
{
    ...
    if(0 != pproto_get(ss, &str_type, sizeof(str_type))) return 1;

    if(PPROTO_LTEXT_STRING_MAGIC == str_type)
    {
        if(0 != pproto_get(ss, (uint8 *)&str_len, sizeof(str_len))) return 1;

        *len = be64toh(str_len);
    }

    if(pproto_get(ss, &state->next_chunk_len, sizeof(state->next_chunk_len)) <= 0)
    {
        return 1;
    }
...
sint8 pproto_read_str(handle ss, uint8 *strbuf, uint64 *sz)
{
    ...
    do
    {
        if(0 != pproto_get(ss, strbuf + read, state->next_chunk_len)) return 1;

        read += state->next_chunk_len;

        if(pproto_get(ss, &state->next_chunk_len, sizeof(state->next_chunk_len)) <= 0) return 1;
    }
...
sint8 pproto_read_str_end(handle ss)
{
    ...
    if(state->next_chunk_len != 0)
    {
        if(pproto_send(ss, &magic, sizeof(magic)) != 0) return 1;
        if(pproto_flush_send(ss) != 0) return 1;

        do
        {
            sz = 256;
            if(0 != pproto_read_str(ss, strbuf, &sz)) return 1;
        }
        while(sz != 0);
    }
    return 0;
...
sint8 pproto_read_date_value(handle ss, uint64 *date)
{
    ...
    if(0 != pproto_get(ss, (uint8 *)&dtn, sizeof(dtn))) return 1;
...
sint8 pproto_read_decimal_value(handle ss, decimal *d)
{
    ...
    if(0 != pproto_get(ss, &b, sizeof(b))) return 1;
    ...
    if(len > DECIMAL_PARTS * 2) return 1;

    if(0 != pproto_get(ss, m, len)) return 1;

    while(i < len / 2)
    {
        d->m[i] = be16toh(*((uint16*)m + i));
        i++;
    }
    ...
    d->sign = (b & 0x80) ? DECIMAL_SIGN_POS : DECIMAL_SIGN_NEG;

    if(b & 0x40)
    {
        if(0 != pproto_get(ss, (uint8 *)&d->e, sizeof(uint8))) return 1;
    }
...
sint8 pproto_read_double_value(handle ss, float64 *d)
{
    ...
    if(0 != pproto_get(ss, (uint8 *)&l, sizeof(l))) return 1;
...
sint8 pproto_read_timestamp_with_tz_value(handle ss, uint64 *ts, sint16 *tz)
{
    ...
    if(0 != pproto_get(ss, (uint8 *)&l, sizeof(l))) return 1;
    if(0 != pproto_get(ss, (uint8 *)&t, sizeof(t))) return 1;
...
pproto_msg_type pproto_read_msg_type(handle ss)
{
    ...
    if(pproto_get(ss, &byte, sizeof(byte)) != 0)
    {
...
sint8 pproto_read_auth_responce(handle ss, uint8 *auth_status)
{
    ...
    if(0 == pproto_get(ss, &status, sizeof(status)))
    {
...
sint8 pproto_send_client_hello(handle ss, encoding client_encoding)
{
    ...
    if(0 != pproto_send(ss, (uint8 *)&magic, sizeof(magic))) return 1;
    if(0 != pproto_send(ss, (uint8 *)&enc, sizeof(enc))) return 1;

    return pproto_flush_send(ss);
}
...
sint8 pproto_send_auth(handle ss, auth_credentials *cred)
{
    ...
    if(0 != pproto_send(ss, &magic, sizeof(magic))) return 1;

    if(0 != pproto_send_string(ss, (const char*)cred->user_name)) return 1;

    if(0 != pproto_send(ss, cred->credentials, AUTH_CREDENTIAL_SZ * sizeof(uint8))) return 1;
...
sint8 pproto_sql_stmt_begin(handle ss)
{
    ...
    return pproto_send(ss, magics, sizeof(magics));
}
...
sint8 pproto_send_sql_stmt(handle ss, const uint8* data, uint32 sz)
{
    ...
    while(sz > 0)
    {
        len = (sz > 256) ? 256 : sz;
        if(pproto_send(ss, &len, sizeof(len)) != 0) return 1;
        if(pproto_send(ss, data, len*sizeof(uint8)) != 0) return 1;
        data += len;
        sz -= len;
    }
...
{
    uint8 terminator[2] = {0, 0};
    if(0 != pproto_send(ss, terminator, sizeof(terminator))) return 1;
